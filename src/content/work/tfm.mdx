...
import EvidenceLadder from "../../components/figures/EvidenceLadder.astro";
import AccuracyResult from "../../components/figures/AccuracyResult.astro";
...
- **Model-agnostic and portable.** Providers sit behind a single interface, so any lab
  ...
  one. The model it ships with was chosen after benchmarking many candidates against a
  hand-curated gold set, not by default.

## Status, and where it's going

The thesis is in progress, and I'm deliberately not posting the catalogue findings yet.
**What the machinery does is a different question, and that one I can answer:**

<AccuracyResult />

If the findings hold, the extended catalogue and the classifier are intended to be
published as part of the **PlantaeViz** platform — including a web app where a researcher
uploads a paper and gets evidence-graded classifications back, ready to review and
correct.

What I can talk through in full is **how it was built** — the design decisions,
the alternatives they discarded, and what each one cost. Happy to go into any of
it before the defence.
